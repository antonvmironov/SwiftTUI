...
///   protocol.
protocol PrimitiveNodeViewBuilder: NodeViewBuilder {}

extension PrimitiveNodeViewBuilder {
    public var body: Never { fatalError("Cannot evaluate body of primitive view") }
}

extension View {
    static var size: Int? {
        if let I = Self.self as? PrimitiveNodeViewBuilder.Type {
            return I.size
        }
        return Body.size
    }
...
extension View {
    var view: NodeViewBuilder {
        if let PrimitiveNodeViewBuilder = self as? PrimitiveNodeViewBuilder {
            return PrimitiveNodeViewBuilder
        }
        return ComposedNodeViewBuilder(view: self)
    }
}
